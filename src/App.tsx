...
interface Employee {
  ...
  email: string;
  department: string;
  role: 'Employee' | 'Asset Manager' | 'Department Head';
...
export default function App() {
  ...
  // Pre-seeded Memory Database States
  ...
  const handlePromoteRole = (email: string) => {
    setEmployees(prev => prev.map(emp => {
      if (emp.email === email) {
        const nextRole = emp.role === 'Employee' ? 'Asset Manager' : emp.role === 'Asset Manager' ? 'Department Head' : 'Employee';
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-[#0c0c0c] text-white font-sans selection:bg-brand/30 selection:text-white">
      ...
      <main className="relative z-10 w-full max-w-6xl mx-auto px-6 py-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 shrink-0 flex flex-col gap-6">
          <div className="premium-glass p-5 rounded-2xl border border-white/5 text-left">
            ...
            <div className="space-y-1">
              {[
                ...
                { id: 'allocations', label: 'Allocations & Buffer', icon: Clock },
                { id: 'maintenance', label: 'Maintenance Requests', icon: Wrench },
                { id: 'audit', label: 'Physical Audit (QR)', icon: ClipboardCheck },
                { id: 'logs', label: 'Compliance Delta Logs', icon: History },
              ].map(tab => (
                <button
                  key={tab.id}
          ...
          {/* Quick Actions (Killer Feature entry point) */}
          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
                ...
                {/* Overdue returns warnings (Rule E & Core constraint) */}
            ...
            <div className="space-y-6">
              <div className="premium-glass p-5 rounded-2xl border border-white/5 text-left">
                <div className="flex justify-between items-center pb-4 border-b border-white/5 mb-4">
                  ...
                  <span className="text-[10px] text-zinc-500 font-mono">Count: {assets.length} items</span>
                </div>
